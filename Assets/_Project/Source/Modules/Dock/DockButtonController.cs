...
namespace Bob.SharedMobility
{
    public class DockButtonController : MonoBehaviour
    {
        ...
        [Tooltip("请把场景里的 MapSystem (挂着 MapViewController 的物体) 拖到这里！\n只有 Home 按钮需要填这个。")]
        ...
        private void TriggerLogic(CanvasGroup subMenu)
        {
            ...
            if (isHomeButton)
            {
                ...
                else
                {
                    ...
                    Debug.LogWarning("⚠️ Home 按钮未绑定 MapViewController！无法重置地图状态。请在 Inspector 中赋值。");
                }
            }
        ...
        void OnBobLeave()
        {
            ...
            if (shouldBobReturn && BobInteractionDirector.Instance)
            {
                ...
                BobInteractionDirector.Instance.ReleaseBobFrom(exitPos);
            }
        }
